...
const checkEmail = (req, res) => {
  user.find({ email: req.body.email }, function (err, user) {
    ...
    if (user.length > 0) {
      ...
      res.status(200).send("Available email for use");
    }
  });
};
...
const updateDetails = (req, res) => {
  ...
  user.updateOne({ _id: req.params._id }, details_update, function (err, user) {
    if (err) {
      res.status(400).send(err);
    }
    res.status(200).json(user);
...
const updatePhoto = (req, res) => {
  ...
  user.updateOne({ _id: req.params._id }, photo_update, function (err, user) {
    if (err) {
      res.status(400).send(err);
    }
    res.status(200).json(user);
